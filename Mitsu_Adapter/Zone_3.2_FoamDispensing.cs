...
namespace SOPS.Mitsu_Adapter
{
	...
	internal class Z32_FoamDispensing : MitsuBaseClass
	{
		...
		public Z32_FoamDispensing(int pLCLogicalStation, int adapterPortNumber, int queryIntervalinMS) : base(pLCLogicalStation, adapterPortNumber, queryIntervalinMS)
		{
		...
		{
			const int userreg = 13026;
			const int opshift = 13043;
			string userdata = string.Empty;
			string shift = string.Empty;


			int SI_No = 0;
			_mitsuPLC.GetDevice("D13022", out SI_No);
			...
			int cAservospeed = 0;
			_mitsuPLC.GetDevice("D13060", out cAservospeed);

			int cAtankspeed = 0;
			_mitsuPLC.GetDevice("D13062", out cAtankspeed);

			int cAtank = 0;
			_mitsuPLC.GetDevice("D13064", out cAtank);
			float cAtanklevel = BitConverter.ToSingle(BitConverter.GetBytes(cAtank), 0);

			int cAop = 0;
			_mitsuPLC.GetDevice("D13066", out cAop);
			float cAoutletpr = BitConverter.ToSingle(BitConverter.GetBytes(cAop), 0);

			int cBservospeed = 0;
			_mitsuPLC.GetDevice("D13068", out cBservospeed);

			int cBtankspeed = 0;
			_mitsuPLC.GetDevice("D13070", out cBtankspeed);

			int cBtank = 0;
			_mitsuPLC.GetDevice("D13072", out cAtank);
			float cBtanklevel = BitConverter.ToSingle(BitConverter.GetBytes(cBtank), 0);

			int cBop = 0;
			_mitsuPLC.GetDevice("D13074", out cBop);
