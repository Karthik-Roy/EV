...
namespace SOPS.Mitsu_Adapter
{
    ...
    internal class Z32_ProductionData : MitsuBaseClass
    {
        ...
        public Z32_ProductionData(int pLCLogicalStation, int adapterPortNumber, int queryIntervalinMS) : base(pLCLogicalStation, adapterPortNumber, queryIntervalinMS)
        {
        ...
        {
            const int userreg = 14249;
            const int opshift = 14281;
            string userdata = string.Empty;
            string shift = string.Empty;

            int SI_No = 0;
            _mitsuPLC.GetDevice("D14245", out SI_No);
            ...
            int zfixoutcount = 0;
            _mitsuPLC.GetDevice("D14313", out zfixoutcount);

            int weldst01count = 0;
            _mitsuPLC.GetDevice("D14314", out weldst01count);

            int weldst02count = 0;
            _mitsuPLC.GetDevice("D14315", out weldst02count);

            int weldintoutcount = 0;
            _mitsuPLC.GetDevice("D14316", out weldintoutcount);

            int foamstationcount = 0;
            _mitsuPLC.GetDevice("D14317", out foamstationcount);

            int thermalstationoutcount = 0;
            _mitsuPLC.GetDevice("D14318", out thermalstationoutcount);

            int bmsactivationcount = 0;
            _mitsuPLC.GetDevice("D14319", out bmsactivationcount);

            int inserationcount = 0;
            _mitsuPLC.GetDevice("D14320", out inserationcount);

            int pulltestcount = 0;
            _mitsuPLC.GetDevice("D14321", out pulltestcount);

            int leaktestcount = 0;
            _mitsuPLC.GetDevice("D14322", out leaktestcount);

            int breathercount = 0;
            _mitsuPLC.GetDevice("D14323", out breathercount);
