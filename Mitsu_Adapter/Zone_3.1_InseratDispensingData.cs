...
namespace SOPS.Mitsu_Adapter
{
	...
	internal class Z31_InseratDispensing : MitsuBaseClass
	{
		...
		public Z31_InseratDispensing(int pLCLogicalStation, int adapterPortNumber, int queryIntervalinMS) : base(pLCLogicalStation, adapterPortNumber, queryIntervalinMS)
		{
		...
		{
			const int userreg = 7004;
			const int opshift = 7021;
			string userdata = string.Empty;
			string shift = string.Empty;


			int SI_No = 0;
			_mitsuPLC.GetDevice("D7000", out SI_No);
			...
			int cAservospeed = 0;
			_mitsuPLC.GetDevice("D7038", out cAservospeed);

			int cAtankspeed = 0;
			_mitsuPLC.GetDevice("D7040", out cAtankspeed);

			int cAtank = 0;
			_mitsuPLC.GetDevice("D7042", out cAtank);
			float cAtanklevel = BitConverter.ToSingle(BitConverter.GetBytes(cAtank), 0);

			int cAop = 0;
			_mitsuPLC.GetDevice("D7044", out cAop);
			float cAoutletpr = BitConverter.ToSingle(BitConverter.GetBytes(cAop), 0);

			int cBservospeed = 0;
			_mitsuPLC.GetDevice("D7046", out cBservospeed);

			int cBtankspeed = 0;
			_mitsuPLC.GetDevice("D7048", out cBtankspeed);

			int cBtank = 0;
			_mitsuPLC.GetDevice("D7050", out cBtank);
