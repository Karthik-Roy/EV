...
namespace SOPS.Mitsu_Adapter
{
	...
	internal class Z31_RealTimeData : MitsuBaseClass
	{
		...
		public Z31_RealTimeData(int pLCLogicalStation, int adapterPortNumber, int queryIntervalinMS) : base(pLCLogicalStation, adapterPortNumber, queryIntervalinMS)
		{
		...
		private void GetRealTimeData()
		{
			...
			int zfixstatus = 0;
			_mitsuPLC.GetDevice("D14173", out zfixstatus);

			int weldst01status = 0;
			_mitsuPLC.GetDevice("D14175", out weldst01status);

			int weldst02status = 0;
			_mitsuPLC.GetDevice("D14177", out weldst02status);

			int weldintstatus = 0;
			_mitsuPLC.GetDevice("D14179", out weldintstatus);

			int foamstationstatus = 0;
			_mitsuPLC.GetDevice("D14181", out foamstationstatus);

			int thermalstationstatus = 0;
			_mitsuPLC.GetDevice("D14183", out thermalstationstatus);

			int bmsactivationstatus = 0;
			_mitsuPLC.GetDevice("D14185", out bmsactivationstatus);

			int inserationstatus = 0;
			_mitsuPLC.GetDevice("D14187", out inserationstatus);

			int buffstatus = 0;
			_mitsuPLC.GetDevice("D14189", out buffstatus);

			int palletstatus = 0;
			_mitsuPLC.GetDevice("D14191", out palletstatus);
